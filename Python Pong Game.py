...
class PONG:
    ...
    def game(self):
        while True:
            ...
            if (210 < self.ball.xcor() < 220) and \
               (self.right_paddle.ycor() - 40 < self.ball.ycor() < self.right_paddle.ycor() + 40):
                self.ball.setx(210)
                self.ball_direction_x *= -1

            if (-220 < self.ball.xcor() < -210) and \
               (self.left_paddle.ycor() - 40 < self.ball.ycor() < self.left_paddle.ycor() + 40):
                self.ball.setx(-210)
                self.ball_direction_x *= -1
